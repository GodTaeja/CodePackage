...
{
    public static Pool<T> Create<T>(Func<T> creatFunc, Action<T> onGet, Action<T> onPut, Action<T> onDestroy = null, int initSize = 4)
    {
        if (creatFunc == null)
            throw new Exception("createFunc must not be null!");

        var pool = new Pool<T>(creatFunc, onGet, onPut, onDestroy, initSize) { ExpandRule = PoolExpandRule.InitSize };
        return pool;
    }
...
public class Pool<T>
{
    ...
    public PoolFactory.PoolExpandRule ExpandRule { get; set; } = PoolFactory.PoolExpandRule.One;

    internal Pool(Func<T> creatFunc, Action<T> onGet, Action<T> onPut, Action<T>onDestroy, int initSize = 32)
    {
        _initSize = initSize;
