	...
	public class RTree<T>
	{
		...
		private void Insert(RTreeNode<T> item, int level)
		{
            ...
            if(node.Envelope==null)
                node.Envelope=new Envelope(envelope.X1, envelope.Y1, envelope.X2, envelope.Y2);
		...
		private RTreeNode<T> ChooseSubtree(Envelope bbox, RTreeNode<T> node, int level, List<RTreeNode<T>> path)
		{
			while (true)
			{
				...
				var minArea = FP.MaxValue;
				var minEnlargement = FP.MaxValue;
		...
		{
			var retval = new Envelope(node.Children[startIndex].Envelope.X1,
                node.Children[startIndex].Envelope.Y1,
                node.Children[startIndex].Envelope.X2,
                node.Children[startIndex].Envelope.Y2);
		...
		private static FP AllDistMargin(RTreeNode<T> node, int m, int M, Comparison<RTreeNode<T>> compare)
		{
        ...
        public RTreeNode<T> Nearest(FP x, FP y)
        {
            //FP distance = FP.MaxValue;
            return Nearest(x, y, root, FP.MaxValue);
        }
        public RTreeNode<T> Nearest(FP x, FP y, FP distance)
        {
            //FP distance = FP.MaxValue;
            return Nearest(x, y,root, distance);
        }

        private static RTreeNode<T> Nearest(FP x,FP y, RTreeNode<T> n, FP nearestDistance)
        {
            RTreeNode<T> nextNode = null;
            //UnityEngine.Debug.LogWarning("n.Children.Count:" + n.Children.Count);
            for (int i = 0; i < n.Children.Count; i++)
            {
                FP tempDistance = n.Children[i].Envelope.Distance(x,y);
                if (n.IsLeaf)
                ...
                { // for index nodes, only go into them if they potentially could have
                    ...
                    if (tempDistance <= nearestDistance)
                    {
                        ...
                        RTreeNode<T> _n = Nearest(x, y, n.Children[i], nearestDistance);
                        if (_n != null)
                        {
                            nearestDistance = _n.Envelope.Distance(x, y);
                            nextNode = _n;
                        }
                    }
                }
        ...
        private static Color[] colors = new Color[] { Color.white,Color.red,Color.blue,Color.cyan,Color.green,Color.magenta,Color.yellow };
        public void DrawNode(RTreeNode<T> node,UnityEngine.Color color)
        {
            //UnityEngine.Debug.LogWarning("height:"+ node.Height+" "+node.Children.Count+" "+node.IsLeaf+" "+node.Envelope);
            UnityEngine.Debug.DrawLine(new Vector3(node.Envelope.X1.AsFloat(), 5+ node.Height, node.Envelope.Y1.AsFloat()),  new Vector3(node.Envelope.X1.AsFloat(), 5 + node.Height, node.Envelope.Y2.AsFloat()), colors[node.Height%colors.Length], 0.1f);
            UnityEngine.Debug.DrawLine(new Vector3(node.Envelope.X1.AsFloat(), 5 + node.Height, node.Envelope.Y2.AsFloat()), new Vector3(node.Envelope.X2.AsFloat(), 5 + node.Height, node.Envelope.Y2.AsFloat()), colors[node.Height % colors.Length], 0.1f);
            UnityEngine.Debug.DrawLine(new Vector3(node.Envelope.X2.AsFloat(), 5 + node.Height, node.Envelope.Y2.AsFloat()), new Vector3(node.Envelope.X2.AsFloat(), 5 + node.Height, node.Envelope.Y1.AsFloat()), colors[node.Height % colors.Length], 0.1f);
            UnityEngine.Debug.DrawLine(new Vector3(node.Envelope.X2.AsFloat(), 5 + node.Height, node.Envelope.Y1.AsFloat()), new Vector3(node.Envelope.X1.AsFloat(), 5 + node.Height, node.Envelope.Y1.AsFloat()), colors[node.Height % colors.Length], 0.1f);
            foreach (var child in node.Children)
            {
                DrawNode(child,color);
            }
        }
#endif
    }
