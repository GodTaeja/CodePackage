...
namespace ILRuntime.CLR.TypeSystem
{
    public class ILType : IType
    {
        ...
        //private ConcurrentBag<ILType> concurrentGenericInstances;
        private ConcurrentDictionary<int, List<ILType>> genericInstancesDict;
        bool isDelegate;
                    ...
                    if (baseType is CLRType)
                    {
                        ...
                        else
                        {
                            ...
                            if (appdomain.CrossBindingAdaptors.TryGetValue(baseType.TypeForCLR, out adaptor))
                            {
                                ...
                                throw new TypeLoadException($"跨域继承没有适配器, {Name}:{baseType.TypeForCLR}");
                        }
                    }
                }
            }
        ...
        {
            if (genericInstancesDict == null)
            {
                genericInstancesDict=new ConcurrentDictionary<int, List<ILType>>();
                //genericInstances = new List<ILType>();
                //concurrentGenericInstances=new ConcurrentBag<ILType>();
            }

            List<ILType> genericInstances = genericInstancesDict.GetOrAdd(Thread.CurrentThread.ManagedThreadId,new List<ILType>());
