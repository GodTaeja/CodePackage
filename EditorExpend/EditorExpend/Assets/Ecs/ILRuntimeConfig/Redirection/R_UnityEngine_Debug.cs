...
public class R_UnityEngine_Debug 
{
    public static unsafe void Register(AppDomain appDomain)
    {
        var mi = typeof(Debug).GetMethod("Log", new[] { typeof(object) });
        appDomain.RegisterCLRMethodRedirection(mi, UnityEngine_Debug);
    }

   //编写重定向方法对于刚接触ILRuntime的朋友可能比较困难，比较简单的方式是通过CLR绑定生成绑定代码，然后在这个基础上改，比如下面这个代码是从UnityEngine_Debug_Binding里面复制来改的
    //如何使用CLR绑定请看相关教程和文档
    static unsafe StackObject* UnityEngine_Debug(ILIntepreter __intp, StackObject* __esp, IList<object> __mStack, CLRMethod __method, bool isNewObj)
    {
